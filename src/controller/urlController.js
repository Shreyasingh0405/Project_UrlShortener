...
const shortenUrl = async function (req, res) {
    // try {
      ...
      console.log(Object.keys(requestBody).length)
    ...
    //   }
      ...
      const urlTrim = longUrl.trim().length;
      console.log(urlTrim)
      const check=longUrl.split(" ").length
      console.log(check)
      if(check!=1){
      if(urlTrim!=check){
        return res
    ...
    //   const urlTrim = urlTrim
        // .split("")
        // .map((a) => a.trim())
        // .join("");
  
      const findInCache = await GET_ASYNC(`${urlTrim}`);
      if (findInCache) {
        let data = JSON.parse(findInCache);
        return res
          .status(200)
          .send({ status: true, message: "Entry from cache", data: data });
      }
      const isAlreadyReg = await urlModel
        .findOne({ longUrl })
        .select({ createdAt: 0, updatedAt: 0, __v: 0 });
      if (isAlreadyReg) {
        await SET_ASYNC(`${urlTrim}`, JSON.stringify(isAlreadyReg));
        res
          .status(200)
          .send({
            status: true,
